...
/**
 * Converte apenas o que importa do RTF (negrito/itálico) em tags,
 * ignorando o resto dos control words.
 * - \b / \b0 => <strong> ... </strong>
 * - \i / \i0 => <em> ... </em>
 * Respeita escopo por grupos { ... }.
 */
function rtfInlineStrongEm(rtf) {
  let i = 0;
  let out = "";
  ...
  const applyState = (nextBold, nextItalic) => {
    // fecha na ordem inversa
    if (italic && !nextItalic) closeItalic();
    if (bold && !nextBold) closeBold();

    // abre na ordem normal
    if (!bold && nextBold) openBold();
    if (!italic && nextItalic) openItalic();
  };
  ...
  while (i < rtf.length) {
    ...
    if (ch === "}") {
      ...
      applyState(prev.bold, prev.italic);
      i++;
      continue;
    }
    ...
    if (ch === "\\") {
      ...
      // parâmetro opcional (ex: \b0, \i0, \fs24)
      let param = "";
      ...
      // espaço delimitador (RTF costuma ter)
      if (j < rtf.length && rtf[j] === " ") j++;

      // trata só o que interessa
      if (word === "b") {
        if (param === "0") applyState(false, italic);
        else applyState(true, italic);
        i = j;
        continue;
      }

      if (word === "i") {
        if (param === "0") applyState(bold, false);
        else applyState(bold, true);
        i = j;
        continue;
      }
      ...
      // ignora o resto dos control words (não emite nada)
      i = j;
      continue;
    }
...
export default {
  async fetch(request) {
    ...
    // remove lixo de hyperlink e afins (mantém igual)
    ...
    // remove grupos pesados antes (mantém igual)
    ...
    // ✅ NOVO: injeta <strong>/<em> preservando o escopo de { }
    texto = rtfInlineStrongEm(texto);

    // a partir daqui, seu pipeline continua (mantido ao máximo)
...

    {
      ...
      if (isJwLink) {
        linkJwOrgParaModulo7 = ultimaLinha;
